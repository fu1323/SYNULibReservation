...
26.9更新: 座位预约支持优先级（id后面的数字，越小越优先）排序 可不限数量  当座位被占用时会根据优先级自动顺延尝试，本次修复了当优先级序号出现跳过时程序会出现非预期行为的问题，配置文件格式进行了更新，(旧版本程序生成配置文件，新版无法兼容，变化不大 但请重新整理)支持备注字段，可以备注座位号 方便人工阅读 修改